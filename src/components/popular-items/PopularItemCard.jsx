...
const PopularItemCard = () => {
    ...
    return (
        ...
        <div className="w-full mx-auto px-1 sm:px-2 md:px-20 lg:px-44">
            <h2 className="mb-2  text-xl sm:text-[29px] font-semibold text-center flex items-center justify-center">
                ...
                {popularItems && popularItems.map((item) => {
                    ...
                    return (
                        <SwiperSlide key={item.ProductId}>
                            <div className="relative h-auto w-full overflow-hidden rounded-2xl shadow-lg">
                                {
                                    item?.ProductImage && (
                                        <img
                                            loading='lazy'
                                ...
                                <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 text-white">
                                    <h3 className="text-lg  font-bold line-clamp-2">{item.ProductName}</h3>
                                    {/* <p className="mt-1 text-sm line-clamp-2 opacity-90">{item.ProductDescription}</p> */}
                                    <div className="mt-1 inline-flex items-center rounded-md ">
                                        {discountPercent > 0 ? (
                                            <>
                                                <span className="text-base sm:text-lg font-semibold">Rs. {Math.round(afterDiscountPrice)}</span>
